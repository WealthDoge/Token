...
contract WealthDogeToken is IBEP20, Auth {
    
    using SafeMath for uint256;

    string constant _name = "WealthDoge";
    string constant _symbol = "WealthDoge";
    ...
    mapping (address => bool) public botBlockList;
	...
	address marketingAddress = 0x7298911C894e79d06B1527BcEAdb5b99298C66e3; //market wallet
    ...
    function setFees(uint256 newLiqFee, uint256 newMarkingFee) external authorized {
    ...
    function setQuidityReceivers(address newLiquidityReceiver) external authorized {
        autoLiquidityReceiver = newLiquidityReceiver;
    }

    function setSwapBack(bool enableSwapBack, uint256 newSwapBackLimit) external authorized {
    ...
    function checkBot(address sender, address recipient) internal{
        require(!botBlockList[sender] && !botBlockList[recipient],"bot");

        //kill the first block.
        if(launchedAt == block.number){
            if(sender != pair) botBlockList[sender] = true;
            if(recipient != pair) botBlockList[recipient] = true;
        }
    }

    function setTradingStatus(bool newStatus) public onlyOwner {
